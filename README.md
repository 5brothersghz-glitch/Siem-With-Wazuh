...
## Step 7: Install Ollama for AI-Assisted Log Analysis

To enhance log analysis capabilities, I installed Ollama on the Ubuntu server to run a local large language model (LLM) for AI-assisted cybersecurity investigations. Ollama was configured to process security logs generated by Wazuh and Sysmon during attack simulations.

I used Ollama to analyze alerts, summarize security events, identify indicators of compromise (IOCs), and explain suspicious activity observed during testing. By leveraging AI-assisted analysis, I was able to review large amounts of log data more efficiently and gain additional context during incident investigations.

The integration of Ollama with the lab environment improved visibility into attack activity generated by MITRE Caldera and supported a more efficient Security Operations Center (SOC) workflow.
